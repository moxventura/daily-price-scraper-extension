...
(function() {
    const popupHtml = `
      <div id="selectionPopup" style="position: fixed; top: 10px; right: 10px; width: 300px; background: white; border: 1px solid #ddd; padding: 10px; z-index: 10000; font-family: Arial, sans-serif;">
        <h1 style="font-size: 18px; margin: 0 0 10px;">Selection Popup</h1>
        <label for="urlField" style="display: block; margin-bottom: 5px;">URL:</label>
        <input type="text" id="urlField" readonly style="width: 100%; padding: 5px; margin-bottom: 10px; border: 1px solid #ccc;">
        <label for="nameField" style="display: block; margin-bottom: 5px;">Name Selector:</label>
        <input type="text" id="nameField" style="width: 100%; padding: 5px; margin-bottom: 10px; border: 1px solid #ccc;">
        <button id="selectName" style="width: 100%; padding: 5px; margin-bottom: 10px; background-color: #007bff; color: white; border: none; cursor: pointer;">Select Name</button>
        <label for="priceField" style="display: block; margin-bottom: 5px;">Price Selector:</label>
        <input type="text" id="priceField" style="width: 100%; padding: 5px; margin-bottom: 10px; border: 1px solid #ccc;">
        <button id="selectPrice" style="width: 100%; padding: 5px; margin-bottom: 10px; background-color: #007bff; color: white; border: none; cursor: pointer;">Select Price</button>
        <label for="decimalField" style="display: block; margin-bottom: 5px;">Decimal Selector:</label>
        <input type="text" id="decimalField" style="width: 100%; padding: 5px; margin-bottom: 10px; border: 1px solid #ccc;">
        <button id="selectDecimal" style="width: 100%; padding: 5px; margin-bottom: 10px; background-color: #007bff; color: white; border: none; cursor: pointer;">Select Decimal</button>
        <label for="promotionField" style="display: block; margin-bottom: 5px;">Promotion Selector:</label>
        <input type="text" id="promotionField" style="width: 100%; padding: 5px; margin-bottom: 10px; border: 1px solid #ccc;">
        <button id="selectPromotion" style="width: 100%; padding: 5px; margin-bottom: 10px; background-color: #007bff; color: white; border: none; cursor: pointer;">Select Promotion</button>
        <button id="saveSelectors" style="width: 100%; padding: 5px; margin-bottom: 10px; background-color: #28a745; color: white; border: none; cursor: pointer;">Save</button>
        <button id="cancelSelectors" style="width: 100%; padding: 5px; margin-bottom: 10px; background-color:rgb(167, 40, 42); color: white; border: none; cursor: pointer;">Cancel</button>
      </div>
    `;
    ...
    document.getElementById("selectPrice").addEventListener("click", () => {
      startSelectionMode("price");
    });
  
    document.getElementById("selectDecimal").addEventListener("click", () => {
      startSelectionMode("decimal");
    });

    document.getElementById("selectPromotion").addEventListener("click", () => {
      startSelectionMode("promotion");
    });
    ...
    document.getElementById("saveSelectors").addEventListener("click", () => {
      const newSelector = {
        ...
        decimal: document.getElementById("decimalField").value,
        promotion: document.getElementById("promotionField").value,
      };
      chrome.runtime.sendMessage({ action: 'saveSelectors', data: newSelector }, (response) => {
    ...
    document.getElementById("cancelSelectors").addEventListener("click", () => {
      document.getElementById("selectionPopup").remove();
      stopSelectionMode();
    });
  
    function startSelectionMode(type) {
      ...
      function handleClick(event) {
        ...
        const selector = CssSelectorGenerator.getCssSelector(event.target, {
            selectors: ['id', 'tag', 'nthchild', 'nthoftype'],
            blacklist: ['[style]']
          });
        
        document.getElementById(type + "Field").value = selector;
